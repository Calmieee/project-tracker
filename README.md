...
>  
>
> У формы в режиме редактирования недоступен выбор проекта, потому что нет поля в соответствующем роуте. Изменять роут не стал, так как, вероятно, так и задумано.
>
...
>
  >
>
> Сохранение состояния формы включается только для режима создания задачи, чтобы при клике на задачу всегда были актуальные данные и не было расхождений между черновиком и серверными данными.

<br/>

# Обоснование выбора необязательных технологий

 -  TailwindCSS и HeadlessUI использовал исключительно для скорости разработки. 
...
5. TanstackQuery и axios:
	...
	- TanstackQuery по этой же причине. Считаю его полноценным ассинхроным стейт-менедментом и у него очень много преимуществ. Некоторые из них, которые использовались, конечно, готовые состояния ответов и т.д., инвалидация кэша и автоматическое обновления данных на страницах.
6. Eslint и Prettier для дополнительного отлова ошибок, стандартизации кода и поддержания чистоты кодовой базы
...
1. Самое важное - добавить уведомление, которое оповещает пользователя об успешном запросе создания/обновления задачи
...
8. Проверить компоненты на очень частые повторные рендеры и если они есть, то мемоизировать их
