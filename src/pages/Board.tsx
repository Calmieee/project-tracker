...
const Board = () => {
  ...
  const tasksByStatus = useMemo(
    () => ({
      [TaskStatus.BACKLOG]:
        data?.data.tasks.filter((task) => task.status === TaskStatus.BACKLOG) ||
        [],
      ...
      [TaskStatus.DONE]:
        data?.data.tasks.filter((task) => task.status === TaskStatus.DONE) || []
    }),
    [data]
  );

  const { columns, activeTask, onDragStart, onDragEnd, onDragOver } =
