...
const useTaskForm = (onClose: () => void) => {
	...
	const { projectData, userData, isPendingBoards, isPendingUsers } = useProjectsAndUsers();
	...
	const mutation = useTaskMutation({ isEditMode, editMode, taskId, onSuccessCallback: onClose });

	const { register, handleSubmit, reset, watch, formState: { errors, isValid, isDirty } } = useForm<TaskSchemaType>({
